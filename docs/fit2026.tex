...
\AddToShipoutPictureBG{%
  \begin{tikzpicture}[remember picture,overlay]
    \node[rotate=45,scale=6,opacity=0.15] at (current page.center) {DRAFT};
  \end{tikzpicture}%
}
\usepackage{pgfplots}
...
——マルチページ検証と二段階精緻化}
\author{川嶋 力 (Tsutomu Kawashima)\\bon soleil}
\date{Draft — Not yet submitted}

\begin{abstract}
本稿では，LLMを設計時のXPathコンパイラとして活用し，
生のURLから再利用可能なXPath式の生成を自動化するシステムXPathGenieを提案する．
...
Auto & 7/12 & 0.500 & 0.857 \\
University & 4/12 & 0.246 & 0.737 \\
NBA Player & 8/12 & 0.333 & 0.500 \\
Book & 6/10 & 0.157 & 0.262 \\
Camera & 2/9 & 0.015 & 0.067 \\\hline
\textbf{全体} & \textbf{40/87} & \textbf{0.317} & \textbf{0.689} \\\hline
\end{tabular}\hfil}
\end{table}
...
(1) \textbf{フィールド発見カバレッジ}——XPathGenieは対象87フィールド中40フィールド（46\%）
についてXPathを生成した．これは抽出精度ではなくフィールド発見の問題であり，
...
（セマンティック構造の欠如により構造保持型圧縮の前提が成立しないレガシーHTML）．
