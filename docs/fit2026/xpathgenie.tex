...
\begin{abstract}
本研究は，LLMを実行時の抽出エンジンではなく設計時のXPathコンパイラとして
再定義することで，Web抽出のコストモデルを変革する．
...
(1) \textbf{フィールド発見カバレッジ}——XPathGenieは対象87フィールド中29フィールド（33\%）
...
本章では，XPathGenieの実装から抽出された設計上の一般原則を整理する．
...
厳密F1=0.317との差は主にフィールド発見カバレッジ（33\%）に起因し，
